...
function renderReglages(){
  ...
  <div class="screen">
    ...
    <div class="card card-pad">
      ...
      <button class="btn btn-ghost" id="migrate-local" style="margin-top:10px">Importer d'anciennes données locales vers le cloud</button>
      <p class="fab-note" style="margin-top:12px;text-align:left">« Sauvegarder » crée un fichier <code>.json</code> de secours (copie de ce qui est dans le cloud) à envoyer par e-mail / WhatsApp. « Importer d'anciennes données locales » ne sert qu'une fois, pour récupérer les visites saisies sur cet appareil avant le passage au cloud.</p>
    </div>
  </div>
  ${tabbar('reglages')}`;
  bindCommon();
...
/* ---------- Migration ponctuelle : ancienne base locale (IndexedDB) → cloud ---------- */
function readLegacyIndexedDB(){
  return new Promise(resolve=>{
    try{
      const rq=indexedDB.open('gembaDB',1);
      rq.onupgradeneeded=e=>{try{e.target.transaction.abort();}catch(_){}}; // pas de base existante : ne pas en créer une vide
      rq.onsuccess=e=>{
        const idb=e.target.result;
        const stores=['templates','visits','actions','settings'];
        const out={};let remaining=stores.length;
        stores.forEach(s=>{
          if(!idb.objectStoreNames.contains(s)){out[s]=[];if(--remaining===0)resolve(out);return;}
          const r=idb.transaction(s).objectStore(s).getAll();
          r.onsuccess=()=>{out[s]=r.result||[];if(--remaining===0)resolve(out);};
          r.onerror=()=>{out[s]=[];if(--remaining===0)resolve(out);};
        });
      };
...
async function migrateLocalToCloud(){
  if(!db){toast('Firebase non configuré');return;}
  toast('Recherche de données locales…');
  const legacy=await readLegacyIndexedDB();
  const total=legacy?['templates','visits','actions','settings'].reduce((n,s)=>n+(legacy[s]||[]).length,0):0;
  if(!total){toast('Aucune donnée locale trouvée sur cet appareil');return;}
  if(!confirm(`Importer ${total} élément(s) trouvé(s) localement sur cet appareil vers le cloud partagé ?`))return;
  for(const s of ['templates','visits','actions','settings'])for(const o of (legacy[s]||[]))await Store.put(s,o);
  await loadCache();toast('Migration terminée');renderReglages();
}
