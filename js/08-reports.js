...
function buildReport(v){
  ...
  let out=`GEMBA WALK — ${v.templateName}\n${v.code||''}\n${v.secteur?'Service : '+v.secteur+'\n':''}Date : ${frDate(v.date)}\nTour fait par : ${v.author||'—'}\n`;
  out+=`Conformité : ${conf==null?'—':conf+'%'}  |  Conforme ${c.ok} · Écart ${c.nok}\n`;
  out+=`\n— ÉCARTS RELEVÉS —\n`;
  let any=false;
...
function visitToXlsx(v){
  ...
  rows.push([L('Service :'),{v:v.secteur||'—'},'',L('Conformité :'),{v:conf==null?'—':conf+'%',s:conf!=null&&conf<70?5:4}]);
  rows.push([L('Conf. / Écart :'),{v:c.ok+' / '+c.nok},'','','']);
  blank();
...
function actionsToXlsx(){
  ...
  const rows=[[Hd('Date visite'),Hd('Service'),Hd('Formulaire'),Hd('Axe'),Hd('Élément'),Hd('Observation'),Hd('Urgence'),Hd('Photo'),Hd('Responsable'),Hd('Échéance'),Hd('Statut'),Hd('En retard')]];
